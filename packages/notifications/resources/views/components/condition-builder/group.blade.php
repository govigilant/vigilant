<div class="w-full border border-base-200 rounded px-2 pt-2 pb-4"  x-data="{ deleteHover: false, addGroupHover: false, addConditionHover: false }"
    ...
    <div class="flex space-x-4 items-center">
        <div class="mt-1 flex gap-2 items-center">
            ...
            <span>
                <label for="group-operator{{$path}}"
                       class="text-sm text-white">@lang('conditions in this group')</label>
            </span>
        </div>
        ...
        <div class="flex-1 flex gap-2 justify-end">
            ...
            <x-form.button class="bg-green hover:bg-green-light" type="button" wire:click="addCondition('{{ $path }}')" x-on:mouseover="addConditionHover = true" x-on:mouseleave="addConditionHover = false">
                @lang('Add Condition')
            </x-form.button>

            <x-form.button class="bg-blue hover:bg-blue-light" type="button" wire:click="addGroup('{{ $path }}')" x-on:mouseover="addGroupHover = true" x-on:mouseleave="addGroupHover = false">
                @lang('Add Group')
            </x-form.button>

            @if(!blank($path))
                <x-form.button class="bg-red hover:bg-red-light" type="button" wire:click="deletePath('{{ $path }}')" x-on:mouseover="deleteHover = true" x-on:mouseleave="deleteHover = false">
                    @lang('Delete Group')
                </x-form.button>
            @endif
        </div>
    ...
    <div>
        ...
        <div class="w-full bg-blue text-white rounded-full py-1 px-2 mt-2" x-cloak x-show="addGroupHover">@lang('New Group')</div>
        <div class="w-full bg-green text-white rounded-full py-1 px-2 mt-2" x-cloak x-show="addConditionHover">@lang('New Condition')</div>
